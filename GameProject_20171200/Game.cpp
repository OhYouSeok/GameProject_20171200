...
bool Game::init(std::string title, int xpos, int ypos, int width, int height, bool fullscreen) {
	if (SDL_Init(SDL_INIT_EVERYTHING) == 0) {
		...
		if (!TheTextureManager::Instance()->load("assets/BackGround.png", "BackGround", renderer)) {
		}
		m_gameObjects.push_back(new BackGrounds(new LoaderParams(0, 0, 640, 480, "BackGround")));
		m_gameObjects.push_back(new Player(new LoaderParams(300, 320, 25, 45, "Player")));
